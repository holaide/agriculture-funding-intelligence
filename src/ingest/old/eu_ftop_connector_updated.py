...
# Official public endpoints from the attached API notes
...
# API keys documented in the attached text
...
def _type_codes_from_flags(config: EuFtopIngestConfig) -> list[str]:
    """
    Based on attached notes:
    - 0 = tenders
    - 1,2,8 = grants/calls classes used by portal examples
    """
    codes = []
...
def _post_search_or_facet(
    ...
    """
    The attached API notes specify:
    - HTTPS POST
    - endpoint includes ?apiKey=SEDIA&text=***
    - query is sent in Form Data as JSON
    - language can be included as parameter
    """
    params = {
        "apiKey": api_key,
        "text": text,
    ...
    # First attempt: multipart/form-data, matching the attached notes
    try:
    ...
    # Second attempt: regular form field
    response = session.post(
        url,
        params=params,
...
def _extract_candidate_records(result: Any) -> list[dict[str, Any]]:
    ...
    # Fallback: if the payload itself looks like one record
    if not candidates and any(k in result for k in ["identifier", "title", "url", "programmePeriod"]):
...
def _normalize_records(records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for rec in records:
        ...
        title = _pick_first(rec, ["title", "name"])
        ...
        status_raw = _pick_first(rec, ["status", "statusLabel"])
        sector_raw = _pick_first(
            rec,
            [
                "topic",
                "theme",
        ...
        publish_date = _parse_date(_pick_first(rec, ["publishDate", "openingDate", "startDate", "publicationDate"]))
        close_date = _parse_date(_pick_first(rec, ["deadlineDate", "closingDate", "deadlineModel", "endDate"]))
        source_url = _pick_first(rec, ["url", "link"])

        if source_url is None and source_record_id:
            source_url = f"https://ec.europa.eu/info/funding-tenders/opportunities/portal/screen/opportunities/topic-details/{source_record_id}"

        keyword_text = " ".join([x for x in [title, description, str(sector_raw) if sector_raw is not None else None] if x])
...
def fetch_eu_ftop(config: EuFtopIngestConfig | None = None) -> pd.DataFrame:
    ...
    # The attached notes document the search endpoint and query structure,
    # but do not document an explicit pagination parameter shape.
    # We therefore repeat the same call up to max_pages only if the payload
    # exposes obvious paged result containers; otherwise we stop after page 1.
    for page_index in range(1, config.max_pages + 1):
        ...
        # Stop unless the payload clearly looks paginated.
        total_count = None
