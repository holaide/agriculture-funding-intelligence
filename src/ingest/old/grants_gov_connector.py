...
def fetch_grants_gov(config: GrantsGovIngestConfig | None = None) -> pd.DataFrame:
    ...
    for page_index in range(1, config.max_pages + 1):
        ...
        response = session.post(GRANTS_SEARCH_URL, json=payload, timeout=config.timeout_seconds, headers={"Content-Type": "application/json"})
        response.raise_for_status()
        result = response.json()

        timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        raw_path = RAW_DIR / f"grants_gov_page_{page_index}_{timestamp}.json"
    ...
    save_parquet_csv(df, STAGING_DIR / "grants_gov_opportunities.parquet", STAGING_DIR / "grants_gov_opportunities.csv")
    return df
