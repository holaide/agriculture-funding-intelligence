...
class GrantsGovIngestConfig:
    ...
    date_windows: list[tuple[int, int]] = field(default_factory=lambda: [
        (2014, 2016),
        (2017, 2019),
        (2020, 2022),
        (2023, 2026),
...
def _build_window_payload(
    ...
    payload = {
        "rows": rows,
        "startRecordNum": ((page_index - 1) * rows) + 1,
        "keyword": term,
        "oppStatuses": [opportunity_status] if opportunity_status else [],
        "sortBy": "openDate|desc",
        "postedFrom": start_date,
        "postedTo": end_date,
        "postDateFrom": start_date,
        "postDateTo": end_date,
        "openDateFrom": start_date,
        "openDateTo": end_date,
    }
    return payload
...
def fetch_grants_gov(config: GrantsGovIngestConfig | None = None) -> pd.DataFrame:
    ...
    for start_year, end_year in config.date_windows:
        window_label = f"{start_year}_{end_year}"
        print(f"=== Fetching Grants.gov window: {start_year}-{end_year} ===")

        for term in config.search_terms:
            query_label = term if term else "all"
            print(f"Fetching Grants.gov records for search term: {query_label!r} in window {start_year}-{end_year}")
            ...
            for page_index in range(1, config.max_pages_per_query + 1):
                payload = _build_window_payload(
                    ...
                    end_year=end_year,
                    opportunity_status=config.opportunity_status,
                )
                ...
                raw_path = RAW_DIR / f"grants_gov_{window_label}_{safe_term}_page_{page_index}_{timestamp}.json"
                ...
                if not opps:
                    print(
                        f"No records returned for term={query_label!r}, "
                        f"window={start_year}-{end_year}, page={page_index}. Stopping this query."
                    )
                    break
                ...
                print(
                    f"window={start_year}-{end_year} term={query_label!r} page={page_index}: "
                ...
                if new_count == 0:
                    print(
                        f"No new unique records for term={query_label!r}, "
                        f"window={start_year}-{end_year}, page={page_index}. Stopping this query."
                    )
                    break
